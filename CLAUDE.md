...
## Recent Changes
- 032-application-filters: Added application filter system with 4 static filter classes (AppFilters, ViFilters, EmacsFilters, SearchFilters) providing IFilter-based state property queries via Condition wrapping Func<bool>, refactored from monolithic AppFilters into domain-specific classes: AppFilters (HasSelection, HasSuggestion, HasCompletions, CompletionIsSelected, IsReadOnly, HasValidationError, HasArg, IsDone, RendererHeightIsKnown, IsMultiline, InPasteMode, InEditingMode with memoization, HasFocus/BufferHasFocus), ViFilters (ViMode, ViNavigationMode, ViInsertMode, ViInsertMultipleMode, ViReplaceMode, ViReplaceSingleMode, ViSelectionMode, ViDigraphMode, ViWaitingForTextObjectMode), EmacsFilters (EmacsMode, EmacsInsertMode, EmacsSelectionMode, IsSearching as EmacsIsSearching), SearchFilters (IsSearching, ControlIsSearchable, ShiftSelectionMode), DummyApplication graceful false for all filters, filter composition via And/Or/Invert, thread-safe via stateless lambdas and Condition caching (6708 tests total, >80% coverage)
