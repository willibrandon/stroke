...
Most recent first. Total test count: **9,333 tests**.

- 068-progressbar-printtext-examples: 24 examples (15 ProgressBar + 9 PrintText) — faithful ports from Python Prompt Toolkit. **ProgressBar shortcut API**: `Stroke.Shortcuts.ProgressBar` class with `IAsyncDisposable`, background UI thread, `Iterate<T>()` counter creation, `ProgressBarCounter<T>` with Label/Total/Percentage/TimeElapsed/TimeLeft properties. **Formatters**: `Text`, `Label`, `Bar`, `Percentage`, `Progress`, `TimeElapsed`, `TimeLeft`, `IterationsPerSecond`, `SpinningWheel`, `Rainbow` in `Stroke.Shortcuts.ProgressBarFormatters`. **ProgressBar examples**: SimpleProgressBar, TwoTasks, UnknownLength, NestedProgressBars, Styled1, Styled2, StyledRainbow, StyledAptGet, StyledTqdm1, StyledTqdm2, ColoredTitleLabel, CustomKeyBindings, ManyParallelTasks (8 concurrent threads), LotOfParallelTasks (160 parallel threads), ScrollingTaskName. **PrintText examples**: AnsiColors, PygmentsTokens, Html, NamedColors, TrueColorDemo, Ansi, PrintFrame, PrintFormattedText, LogoAnsiArt. Bug fixes: KeyboardInterruptException/EOFException clean exit in ProgressBar background thread, VSplit flexible Dimension for large task counts, ProgressBar/ProgressBarWidget name collision in FullScreenDemo
- 065-prompt-examples: 52 prompt examples — faithful ports from Python Prompt Toolkit's `prompts/` directory across 10 categories: Basic Prompts (accept-default, confirmation-prompt, enforce-tty-input-output, get-input, get-input-vi-mode, get-input-with-default, get-multiline-input, get-password, mouse-support, multiline-prompt, no-wrapping, operate-and-get-next, placeholder-text), Password & Security (get-password-with-toggle), Styling & Formatting (bottom-toolbar, clock-input, colored-prompt, cursor-shapes, fancy-zsh-prompt, right-prompt, swap-light-dark-colors, terminal-title), Auto-Suggestion (auto-suggestion, multiline-autosuggest), Auto-Completion (autocompletion, fuzzy-word-completer, control-space-trigger, readline-style, colored-completions, formatted-completions, merged-completers, fuzzy-custom-completer, multi-column, multi-column-with-meta, nested-completion, slow-completions), Key Bindings (autocorrection, custom-key-binding, custom-vi-operator, switch-vi-emacs, system-prompt), History (persistent-history, slow-history, up-arrow-partial-match), Validation & Lexing (custom-lexer, html-input, input-validation, regular-language), Advanced Features (async-prompt, patch-stdout, shell-integration, system-clipboard), Frames (basic-frame, gray-frame-on-accept, frame-with-completion); bug fixes in KeyBindings sort order (AnyCount descending), AppContext generic variance, DynamicCompleter null handling
- 066-system-clipboard: SystemClipboard — cross-platform IClipboard implementation synchronizing with the OS clipboard, faithful port of Python Prompt Toolkit's PyperclipClipboard with inlined platform providers. IClipboardProvider internal interface with five implementations: WindowsClipboardProvider (Win32 P/Invoke), MacOsClipboardProvider (pbcopy/pbpaste), LinuxClipboardProvider (wl-copy/xclip/xsel), WslClipboardProvider (clip.exe/powershell.exe), StringClipboardProvider (test backing). ClipboardProviderDetector auto-detects platform (Windows → macOS → WSL → Wayland → X11). Selection type preservation via _lastData cache with value equality comparison. PlatformUtils.IsWsl property with Lazy<bool> caching. ClipboardProviderNotAvailableException with platform-specific installation guidance. Thread-safe via Lock.EnterScope(). Process-based providers use ArgumentList (not Arguments) for command injection prevention, UseShellExecute=false, CreateNoWindow=true, 5-second timeout. Updated system-clipboard example to use real SystemClipboard instead of InMemoryClipboard
- 064-fullscreen-examples: 25 full-screen examples — faithful ports from Python Prompt Toolkit's `full_screen/` directory: `HelloWorld` (minimal app), `Calculator` (interactive calculator), `Buttons` (button widgets), `TextEditor` (multi-buffer editor with syntax highlighting via TextMateLineLexer), `Pager` (scrollable text viewer), `SplitScreen` (side-by-side text areas), `FullScreenDemo` (feature showcase with menus and completion), `DummyApp` (minimal no-op), `NoLayout` (no layout container), `AnsiArtAndTextArea` (ANSI art with text area); 13 SimpleDemos (Alignment, AutoCompletion, ColorColumn, CursorHighlight, Floats, FloatTransparency, Focus, HorizontalAlign, HorizontalSplit, LinePrefixes, Margins, VerticalAlign, VerticalSplit); 2 ScrollablePanes (SimpleExample, WithCompletionMenu); new library additions: `ILineLexer`/`LineLexer`/`TextMateLineLexer`/`TextMateScopeMapper` for line-based syntax highlighting, `PygmentsStyles` for named style lookup, `MenuContainer`/`MenuItem` widgets for menu bars, async `AcceptHandler` (`Func<Buffer, ValueTask<bool>>`) in Buffer; bug fixes in FloatContainer, Window, HSplit, NumberedMargin, ScrollbarMargin, AsyncGeneratorUtils, NamedCommands.History
- 063-dialogs-examples: 9 dialog examples — faithful ports from Python Prompt Toolkit's `dialogs/` directory: `ButtonDialog` (multiple button options), `CheckboxDialog` (multi-select checkbox list), `InputDialog` (text input), `MessageBox` (simple message), `PasswordDialog` (password input with masking), `ProgressDialog` (background task with file system walk matching Python os.walk behavior), `RadioDialog` (single-select radio list), `StyledMessageBox` (styled message), `YesNoDialog` (boolean confirmation); all dialogs use Enter key to navigate focus from input to OK button via `acceptHandler` pattern; `DefaultKeyBindings.Load()` fix to include Emacs/Vi bindings for proper Enter handling
- 061-ssh-server: StrokeSshServer + StrokeSshSession — faithful port of Python Prompt Toolkit's SSH module using FxSsh v1.3.0, `StrokeSshServer` with host/port/interact/hostKeyPath/encoding/style/enableCpr constructor, `RunAsync()` with readyCallback and CancellationToken, `Connections` snapshot property, graceful shutdown with 5-second timeout; `StrokeSshSession` with GetSize()/DataReceived()/TerminalSizeChanged()/Close() methods, session isolation via AppSession; `ISshChannel` abstraction for testability; `SshChannel` FxSsh adapter with terminal type/size tracking; `SshChannelStdout` TextWriter with LF→CRLF conversion per NVT spec; virtual `BeginAuth()` and `CreateSession()` for extensibility; asyncssh-server example ported (progress dialog, prompts, autocompletion, dialogs); thread-safe via Lock and ConcurrentDictionary per Constitution XI (80 tests, >80% coverage)
- 072-logging: StrokeLogger static class — central logging infrastructure using Microsoft.Extensions.Logging.Abstractions, `Logger` singleton property, `Configure(ILoggerFactory)` for provider setup, `CreateLogger<T>()` and `CreateLogger(string)` for component-specific loggers, `NullLoggerFactory` default for zero-performance-impact when disabled, thread-safe via `System.Threading.Lock`, integrated into telnet classes (TelnetServer, TelnetConnection, TelnetProtocolParser, ConnectionStdout)
- 060-telnet-server: TelnetServer + TelnetConnection — faithful port of Python Prompt Toolkit's telnet module, `TelnetServer` with host/port/interact/encoding/style/enableCpr constructor, `RunAsync()` with readyCallback and CancellationToken, `Connections` snapshot property, connection lifecycle management with 5-second shutdown timeout; `TelnetConnection` with Socket/RemoteAddress/Size/IsClosed properties, `Send(AnyFormattedText)`/`SendAbovePrompt()`/`EraseScreen()`/`Close()` methods, NAWS window size parsing (clamped 1-500), TTYPE terminal type negotiation with 500ms timeout and VT100 fallback; `TelnetProtocolParser` state machine (5 states) with IAC escaping, subnegotiation buffer (1KB limit); `ConnectionStdout` TextWriter with LF→CRLF conversion per NVT spec; `TelnetConstants` with 20+ protocol constants; 4 telnet examples (hello-world, chat-app, dialog, toolbar) faithful ports from Python; thread-safe via Lock and ConcurrentDictionary (208 tests, >80% coverage)
- 059-async-generator-utils: AsyncGeneratorUtils static class — `Aclosing<T>()` wrapper ensuring async generators are properly disposed on early exit/exception, `GeneratorToAsyncGenerator<T>()` converting sync `IEnumerable<T>` to `IAsyncEnumerable<T>` with backpressure via bounded `BlockingCollection<T>`, `IAsyncDisposableValue<out T>` covariant interface combining disposal with value access, `Done` internal sentinel for producer completion, lazy producer thread startup on `GetAsyncEnumerator()`, cooperative cancellation via `volatile bool` flag with 1-second timeout TryAdd, exception propagation from producer to consumer on next `MoveNextAsync()`, disposal terminates producer within 2 seconds (NFR-004), thread-safe per FR-014/FR-015/FR-016
...
- Earlier features (001-050): Project setup, immutable Document, Buffer, Selection, Clipboard, AutoSuggest, Cache, History, Validation, Search, Keys enum, Completion system (12 completers), Mouse events, Input system (Vt100Parser, Win32Input), FormattedText (Html, Ansi, Template), Dimensions, Filter system, Styles, Output system (IOutput, Vt100Output), Editing modes, KeyBindings, Utilities, Lexers, Vi digraphs, Regular languages, Screen/Char, Layout containers (HSplit, VSplit, Float, Window, BufferControl), Application system (Renderer, KeyProcessor, AppContext), Input processors (26), Application filters, Completion menus, Named commands (49 Readline), Scroll/Mouse/Search/AutoSuggest/Focus/CPR/OpenInEditor bindings, Emacs bindings (78+34), Vi bindings (~300), Toolbars (7), Widgets (15), Shortcut utils, PromptSession, Dialog shortcuts, StdoutProxy/PatchStdout, EventLoopUtils
