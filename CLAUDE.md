...
- C# 13 / .NET 10 + Stroke.Application, Stroke.Widgets (Dialog, Label, Button, TextArea, RadioList, CheckboxList, ProgressBar, ValidationToolbar, Box), Stroke.Layout (HSplit, Layout), Stroke.KeyBinding (KeyBindings, MergedKeyBindings, DefaultKeyBindings), Stroke.Filters (FilterOrBool), Stroke.Completion (ICompleter), Stroke.Validation (IValidator), Stroke.FormattedText (AnyFormattedText), Stroke.Styles (IStyle) (048-dialog-shortcuts)
- N/A (stateless factory functions) (048-dialog-shortcuts)

## Recent Changes
- 048-dialog-shortcuts: Dialogs static class (7 factory methods + 7 async wrappers + 2 private helpers), YesNoDialog (bool result with Yes/No handlers), MessageDialog (object? with ReturnNone cancel), InputDialog (string? with TextArea acceptHandler→focus okButton, ValidationToolbar, password/completer/validator support), ButtonDialog<T> (generic buttons via LINQ Select closure capture), RadioListDialog<T?> (RadioList single-select with CurrentValue), CheckboxListDialog<IReadOnlyList<T>?> (CheckboxList multi-select with CurrentValues.AsReadOnly()), ProgressDialog (ProgressBar + TextArea with PreRunCallables→CreateBackgroundTask→Task.Run, Lock-protected setPercentage + _actionChannel-marshaled logText, try/finally app.Exit()), CreateApp<T> private helper (KeyBindings with ControlI/BackTab for Tab/Shift-Tab focus cycling, MergedKeyBindings with DefaultKeyBindings.Load(), mouseSupport: true, fullScreen: true), ReturnNone private helper (AppContext.GetApp().Exit() with no args), all dialogs use Dialog widget with withBackground: true (8271 tests)
