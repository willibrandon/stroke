# Contract: Condition Filter

**Namespace**: `Stroke.Filters`
**Port of**: `prompt_toolkit.filters.base.Condition`

## Class Definition

```csharp
/// <summary>
/// Filter that wraps a <see cref="Func{TResult}"/> callable for dynamic evaluation.
/// </summary>
/// <remarks>
/// <para>
/// This filter evaluates the provided callable each time <see cref="Invoke"/> is called,
/// allowing the filter result to change based on runtime state.
/// </para>
/// <para>
/// Can be used as a factory for creating filters from lambda expressions:
/// <code>
/// var isActive = new Condition(() => someState.IsActive);
/// </code>
/// </para>
/// <para>
/// This is a faithful port of Python Prompt Toolkit's <c>Condition</c> class.
/// </para>
/// </remarks>
public sealed class Condition : Filter
{
    private readonly Func<bool> _func;

    /// <summary>
    /// Initializes a new instance of the <see cref="Condition"/> class.
    /// </summary>
    /// <param name="func">The callable function to evaluate.</param>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="func"/> is <c>null</c>.</exception>
    public Condition(Func<bool> func);

    /// <inheritdoc/>
    /// <returns>The result of evaluating the wrapped function.</returns>
    /// <remarks>
    /// Any exception thrown by the wrapped function will propagate to the caller.
    /// </remarks>
    public override bool Invoke();

    /// <inheritdoc/>
    public override string ToString();
}
```

## Behavioral Contract

### Constructor

- MUST throw `ArgumentNullException` if `func` is `null`
- MUST store the function reference for later evaluation
- MUST call base `Filter` constructor to initialize caches

### Invoke()

- MUST call the wrapped function and return its result
- MUST NOT catch or suppress exceptions from the wrapped function
- MUST NOT cache the result (allows dynamic evaluation)

### ToString()

- MUST return a string representation including the function

## Usage Examples

```csharp
// Simple boolean check
var isActive = new Condition(() => _state.IsActive);

// Combine with other filters
var canEdit = isActive & new Condition(() => !_state.IsReadOnly);

// Use with application state
var hasFocus = new Condition(() => _application.FocusedControl == this);
```

## Thread Safety

- The `Condition` instance itself is thread-safe (immutable after construction)
- Thread safety of the wrapped `Func<bool>` is the caller's responsibility
- If the callable accesses shared mutable state, the caller must ensure thread safety
