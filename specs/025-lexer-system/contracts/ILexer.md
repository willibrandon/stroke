...
public interface ILexer
{
    ...
    /// Used for cache invalidation in <see cref="DynamicLexer"/>.
    /// </returns>
    /// <remarks>
    /// <para>
    /// For stateless lexers like <see cref="SimpleLexer"/>, this typically returns
    /// the lexer instance itself (identity-based comparison).
    /// </para>
    /// <para>
...
```csharp
// Basic usage
ILexer lexer = new SimpleLexer("class:input");
var document = new Document("hello\nworld");
var getLine = lexer.LexDocument(document);

var line0 = getLine(0); // [("class:input", "hello")]
var line1 = getLine(1); // [("class:input", "world")]
var line2 = getLine(2); // [] (empty - beyond bounds)

// Invalidation check
var hash1 = lexer.InvalidationHash();
// ... later ...
var hash2 = lexer.InvalidationHash();
if (!ReferenceEquals(hash1, hash2))
{
    // Re-lex document
}
```
...
1. `LexDocument` never returns `null`
2. The returned function handles any integer input (negative returns empty list)
3. `InvalidationHash()` returns the same value if the lexer's behavior hasn't changed
